    ...
    with encrypt:
        ...
        if upload_file is not None:
            ...
            if choice=="AES":
                ...
                #size=st.selectbox("Choose an key size(in bits)",[128,192,256])
                #size=size//8
                rsa_size=st.selectbox("Choose an RSA key(in bits)",[2048,1024,4096],key="encry_aes_rsa")
    ...
    with decrypt:
        ...
        if upload_file_decrypt is not None:
            ...
            if decrypt_choice=="AES":
                ...
                rsa_size=st.selectbox("Choose an RSA key(in bits)",[2048,1024,4096],key="decry_aes_rsa")
                rsa_path="AES+RSA//"+str(rsa_size)
